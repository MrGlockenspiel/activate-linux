#include <errno.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#include "log.h"
#include "options.h"
#include "osrelease.h"

#define LINE_BUFFER_SIZE 1024

static void copy_value(const char *var, char *line, char *dest) {
  size_t i;
  int j = 0;
  char *buffer;

  buffer = strstr(line, "=") + 1;
  for (i = 0; i < (strlen(line) - strlen(var)); i++) {
    if (buffer[i] == '"')
      continue;
    if (buffer[i] == '\n')
      continue;
    dest[j++] = buffer[i];
  }
  dest[j] = 0;
}

char *get_release_info(void) {
  const char *PRETTY = "PRETTY_NAME";
  const char *SIMPLE = "NAME";

  FILE *osrelease;

  char line[LINE_BUFFER_SIZE];

  __debug__("Attempting to read /etc/os-release\n");
  if ((osrelease = fopen("/etc/os-release", "r")) == NULL) {
    __error__("Error reading /etc/os-release: %s\n", strerror(errno));
    exit(1);
  }

  char *pretty = malloc(sizeof(char) * 1024);
  memset(pretty, 0, 1024);

  char *fallback = malloc(1024);
  memset(fallback, 0, 1024);

  while (fgets(line, sizeof(line), osrelease) != NULL) {
    if (strncmp(PRETTY, line, strlen(PRETTY)) == 0) {
      copy_value(PRETTY, line, pretty);
    }
    if (strncmp(SIMPLE, line, strlen(SIMPLE)) == 0) {
      copy_value(SIMPLE, line, fallback);
    }
  }

  fclose(osrelease);

  if (pretty[0] == 0) {
    free(pretty);
    return fallback;
  } else {
    free(fallback);
    return pretty;
  }
}
